...
export const getPublicDataToken = async () => {
  return fetch(`https://sgisapi.kostat.go.kr/OpenAPI3/auth/authentication.json?consumer_key=${process.env.NEXT_PUBLIC_SGIS_KEY}&consumer_secret=${process.env.NEXT_PUBLIC_SGIS_SECRET}`)
    .then((res) => {
      return res.json();
    })
    .then((json) => {
...
export const getAddressData = async (accessToken: string, cd?: string) => {
  return fetch(`https://sgisapi.kostat.go.kr/OpenAPI3/addr/stage.json?accessToken=${accessToken}${cd ? `&cd=${cd}` : ''}`)
    .then((res) => {
      return res.json();
    })
    .then((json) => {
