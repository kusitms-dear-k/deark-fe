...
const BottomStepperControls = ({ step = 1, title, content1, content2, onNext, onSkip, buttonContent = '다음', skipButtonContent = '우선 둘러볼래요' }: BottomStepperControlsProps) => {
  return (
    <div className="w-full flex flex-col items-center justify-center ">
      <section className="absolute  bottom-72 h-[100px] bg-gradient-to-t from-bg-100 to-transparent w-full"></section>
      <footer className="absolute bg-bg-100 w-full bottom-10 px-5 flex flex-col items-center justify-center gap-y-10">
        ...
        <section className="flex flex-col items-center justify-center w-full flex-col gap-y-3">
          ...
          <div className="flex gap-x-2">
            ...
            <button onClick={onSkip}
                    className={`${skipButtonContent === '로그인' ? 'text-blue-400' : 'text-gray-400'} body-m underline`}>{skipButtonContent}</button>
          </div>
        </section>
      </footer>
    </div>
